...
def read_file_csv(filename):
    print(f'Iniciando la carga del archivo {filename}...')
    ...
    grid_search = GridSearchCV(pipeline, param_grid, cv=5, scoring='roc_auc', verbose=3)  # Agrega verbose para seguimiento
    grid_search.fit(X_train, y_train)

    print('Modelo entrenado correctamente.')

    # Guardamos el modelo entrenado para usarlo en producción
    package = '../models/best_model.pkl'
    pickle.dump(grid_search, open(package, 'wb'))
    print(f'Modelo exportado correctamente en {package}')
